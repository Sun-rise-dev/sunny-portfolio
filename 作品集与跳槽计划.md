# sunny-portfolio 叙事升级 + 跳槽计划

> **最新（2026-07）**：按《求职执行清单》转向 **AI 应用落地 / 实施交付（Junior）**；B2B 预约案例（中邮/康养）与智能客服案例同步整理中。

---

## 已写入站点 · 中医诊所智能客服（光墉堂 · 小墉）

**状态：** 已录入 `src/cases.ts`（`clinic-agent`）与 `src/content.ts`（Agents 首位）；**面试可现场演示**完整链路（专用对话脚本已备）。

### 项目一句话

> Coze 智能体（Prompt + RAG）采集预约意向 → 飞书智能体推送结构化「新预约通知」→ 前台人工确认；预留公域（抖音等）/ ERP 接口。
...
```
项目：某中医诊所 · 智能客服与预约通知（Coze + 飞书）
角色：Agent 配置与交付集成
职责：
- 配置 Coze 智能体：Prompt、知识库 RAG、对话边界与预约字段采集
- 对接飞书智能体，将预约意向推送为结构化通知卡片
- 完成多轮对话验证；预留 webhook 供公域渠道或 ERP 接入
交付物：可演示完整链路 + 飞书通知截图（脱敏）
边界：对信息采集准确、通知可达、前台可跟进负责；不对获客/转化/GMV 负责
```

### 面试演示说明

- 使用**面试专用对话脚本**，可走通：咨询 → 采集 → 触发飞书通知
- 无需公开 Bot 链接；现场演示即可
- 可对照站点 **Cases → 某中医诊所 · 智能客服** 与 **Agent → 光墉堂智能客服 · 小墉**

### 物证清单

- [x] 飞书「新预约通知」截图（公开前打码手机号）
- [x] 站点案例页 + Agent 页已录入
- [ ] 对话录屏（可选备份）

---

## 历史文档 · 连锁/SaaS 叙事升级（待与清单对齐后修订）

> 以下 Phase 1–3 为早期「连锁/SaaS 顾问」方向，**与当前求职清单部分冲突**（勿写 Rollout 已落地、营收当标题）。保留供参考，实施前以清单为准。
